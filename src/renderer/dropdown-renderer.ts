import { DropdownComponent, TextAreaComponent, TextComponent, ToggleComponent } from 'obsidian';
...
export class DropdownRenderer<T extends Record<string, any>> extends AbstractPathRenderer<T> {
  constructor(context: Context<T>, groupMember: boolean, protected element: Dropdown<T>) {
  ...
  protected createElement() {
    ...
    this.setting.addDropdown((dropdown) => {
      ...
      htmlComponent = dropdown.selectEl;
    });
    return { valueComponent: valueComponent, htmlElement: htmlComponent };
  }

  protected renderClassSpecific(): void {
