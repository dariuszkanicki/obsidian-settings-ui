import { FlatPropertyMeta, mergeProperties, parseInitialTypes, PropertyMeta, TypeMeta } from './parse-types.js';

export function parseTypesFromFile(filePath: string): Record<string, TypeMeta> {
  // 1. Parse the initial types
  const typeMap = parseInitialTypes(filePath);

  // 2. Analyze dependencies
  const analysis = analyzeTypeDependencies(typeMap);
  ...
  // 3. Resolve in optimal order
  const typeMetaMap = resolveTypesInOptimalOrder(typeMap, analysis);

  // 4. Verify results
  // console.log('Fully resolved BaseSetting:', JSON.stringify(typeMetaMap['BaseSetting'], null, 2));

  // // 2. Resolve all references
  // const typeMetaMap = resolveTypeReferences(typeMap);

  // 3. Now you can access fully resolved types
  // console.log(resolvedTypes['BaseSetting']);
  return typeMetaMap;
}

function analyzeTypeDependencies(typeMap: Record<string, TypeMeta>) {
...
function resolveTypesInOptimalOrder(typeMap: Record<string, TypeMeta>, analysis: ReturnType<typeof analyzeTypeDependencies>) {
  ...
  for (const typeName of resolutionOrder) {
    ...
    for (const baseType of baseTypes) {
      ...
      if (type.extends.includes('&')) {
        // Intersection: merge properties
        resolvedProperties = mergeProperties(resolvedProperties, baseProperties);
      } else if (type.extends.includes('|')) {
        // Union: mark properties as optional
        const optionalBaseProps = baseProperties.map((p) => ({
          ...p,
        ...
        resolvedProperties = mergeProperties(resolvedProperties, optionalBaseProps);
      }
    }
  ...
  for (const typeName in typeMap) {
    typeMap[typeName].flatProperties = flattenProperties(typeName, typeMap);
  }

  return typeMap;
}

function flattenProperties(typeName: string, typeMap: Record<string, TypeMeta>, visited = new Set<string>()): FlatPropertyMeta[] {
  ...
  const flatPropsMap = _createWithDirectProps(typeMeta);
  ...
  if (typeMeta.extends) {
    ...
    for (const baseType of baseTypes) {
      const baseProps = flattenProperties(baseType, typeMap, new Set(visited));
...
function _createWithDirectProps(typeMeta: TypeMeta): Record<string, FlatPropertyMeta> {
  const flatPropsMap: Record<string, FlatPropertyMeta> = {};
